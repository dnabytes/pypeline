...
class Step:
    ...
    def write_to_log(self, exit_code):
        if self.command_was_changed:
            log.add('**COMMAND WAS CHANGED**')
        log.add(f'* Command: {self.used_command}')
        if self.params:
            log.add(f'* Params: {(", ").join([(" = ").join((param, value)) for param,value in self.params.items()])}')
    ...
    def clean_params_created_files(self):
        self.params = {}
